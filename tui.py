...
def run_tag(a):
    ...
    prog=Progress(SpinnerColumn(spinner_name="dots12"),
    ...
    with Live(console=C, refresh_per_second=8) as live:
        for dp,fn in targets:
            ...
            if a.apply:
                try: os.rename(full,os.path.join(dp,f"[{st}-{su}] {fn}"))
                except Exception: pass
            prog.advance(task)
            live.update(Panel.fit(prog, title=BANNER, subtitle=f"[dim]{fn[:48]}[/]"))
    # log
    ...
    C.print(Panel(bars(counts,len(targets)),
...
def menu():
    C.print(Panel.fit(BANNER, subtitle="[dim]offline · subject+stream tagging[/]"))
    while True:
